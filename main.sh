...
get_omw_latest_version_from_github() {
  curl --retry 2 -I -s https://github.com/lmk123/oh-my-wechat/releases/latest | grep "^[lL]ocation" | sed -n 's/.*\/v\(.*\)/\1/p'
}
...
get_latest_version() {
  curl --retry 2 -I -s https://github.com/MustangYM/WeChatExtension-ForMac/releases/latest | grep "^[lL]ocation" | sed -n 's/.*\/v\(.*\)/\1/p'
}
